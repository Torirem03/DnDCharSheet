...
class interface{
	...
	createCharSheet(){
		...
		//valid input loop
		while (parseInt(otherInput) > 3 || parseInt(otherInput) < 1){
	...
	editCharSheet(){
		...
		//else if edit strength
			...
			activeCharSheet.setStr(tempStr);
		//else if edit dex
			...
			activeCharSheet.setDex(tempDex);
		//else if edit cha
			...
			activeCharSheet.setCha(tempCha);
		//else if edit int
		...
		//else if wis
			...
			activeCharSheet.setWis(tempWis);
		//else if con
			...
			activeCharSheet.setCon(tempCon);
		//else if player name
			...
			activeCharSheet.setPlayer(tempPlayer);
		//else if level
		...
		//else if hair
			...
			activeCharSheet.setHair(tempHair);
		} else {
	...
	saveCharSheet(){
		...
		if (savedCharSheets.empty()){
			...
			for (i = 0; i < savedCharSheets.size(); i++)
	...
	loadCharSheet(){
		...
		for (int i = 0; i < savedCharSheets.size(); i++)
			...
			if(savedCharSheets[i].name == tempName || savedCharSheets[i].class == tempClass || savedCharSheets[i].level == tempLevel){
	...
	//end method
		...
		ifstream charSheetCsv;
   		charSheetCsv.open("characterSheets.csv");
		if(!savedCharSheets.isempty() && charSheetCsv){
		...
		} else if(!savedCharSheets.isempty() && !charSheetCsv){
			//create the csv
